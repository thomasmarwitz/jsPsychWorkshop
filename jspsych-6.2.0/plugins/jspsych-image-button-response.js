  ...
  plugin.trial = function(display_element, trial) {
    ...
    if (trial.render_on_canvas) {
      ...
      for (var i = 0; i < trial.choices.length; i++) {
        ...
        if (i == 1) {
          html += '</div><div class="button-holder"div>'
        } 
      }
      html += "</div>" // to separate the three buttons: 2, 1
      ...
      if (trial.prompt !== null) {
        ...
        display_element.insertBefore(prompt_div, null)
        display_element.insertBefore(canvas, prompt_div.nextElementSibling);
      } else {
    ...
    const btn_handler = (event) => {
      if (answered) {
        return;
      } 
      console.log(event)
      const pressed_key = event.key // key as letter
      const choice = trial.linked_keys?.indexOf(pressed_key) // choice is 0,1,2
      ...
      //console.log(display_element.querySelector("#jspsych-image-button-response-button-" + choice).querySelector(".my-btn"))
      display_element.querySelector("#jspsych-image-button-response-button-" + choice).querySelector(".my-btn").className += " my-btn-clicked"
      
      answered = true
      setTimeout(
        () => after_response(choice), trial.border_delay)
    }

    document.addEventListener('keypress', btn_handler);
    ...
    function after_response(choice) {
      ...
      document.removeEventListener('keypress', btn_handler)
